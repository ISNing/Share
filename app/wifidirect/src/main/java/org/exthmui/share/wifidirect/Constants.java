package org.exthmui.share.wifidirect;

public class Constants {
    public static final String COMMAND_CANCEL = "CANCEL";
    public static final String COMMAND_ACCEPT = "ACCEPT";
    public static final String COMMAND_REJECT = "REJECT";
    public static final String COMMAND_SUCCESS = "SUCCESS";
    public static final String COMMAND_FAILURE = "FAILURE";
    public static final String COMMAND_TRANSFER = "TRANSFER";
    public static final String COMMAND_TRANSFER_END = "TRANSFER_END";

    public static final String SHARE_PROTOCOL_VERSION = "1.0";

    public static final String LOCAL_SERVICE_INSTANCE_NAME = "_share";
    public static final String LOCAL_SERVICE_SERVICE_TYPE = "_presence._tcp";

    public static final String RECORD_KEY_SHARE_PROTOCOL_VERSION = "shareVersion";
    public static final String RECORD_KEY_SERVER_PORT = "serverPort";
    public static final String RECORD_KEY_DISPLAY_NAME = "displayName";
    public static final String RECORD_KEY_PEER_ID = "peerId";
    public static final String RECORD_KEY_UID = "uid";
    public static final String RECORD_KEY_ACCOUNT_SERVER_SIGN = "serverSign";


}
